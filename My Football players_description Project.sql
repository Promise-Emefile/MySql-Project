...
Value_Euro INTEGER NULL,
Wage_Euro  INTEGER NULL,
Team_Position VARCHAR (5) NULL
);

Insert Into Players_Description values (Playerid, 'Lionel Andres', 'Messi', 37, '1987-6-24', 170, 72, 'ARG', 'FC Barcelona', 96, 565, 'RW');
...
# Players that plays for FC Barcelona and Argentina
select Firstname, Lastname, club, nationality
from players_description
where nationality = 'ARG' and club = 'FC Barcelona';

# Players that are from  Argentina Or Brazil or Germany

Select Firstname, Lastname, Nationality
from Players_description 
where Nationality IN ('ARG', 'BRA', 'BRA');

#Players with age 22, 30, and 28
select firstname, lastname, age
from players_description
where Age IN (22, 30, 28)
order by age asc;

#clubs that starts with FC
select club
from players_description 
where club like 'FC%';

#Players that were born between 1990 and 1995
select Firstname, Lastname, Birthdate
from players_description
where birthdate between '1990-01-01' and '1995-12-01';

#Avg age per club
select Club, avg(age) as Avg_age
from players_description
group by club
order by avg_age desc;

#Average height per nationality for players aboves age 28, sorted in asc order
select Avg(height) as Avg_Height, nationality
from players_description 
where age > 28
...
select distinct club, count(firstname) as num_playerPerclub
from players_description
group by club
...
from players_description
group by nationality
order by Nplayer_nationality desc;
